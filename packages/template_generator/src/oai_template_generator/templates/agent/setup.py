...
def discover_scripts():
    ...
    servers_config_directory = os.path.join(package_dir, "agentic_registry_agents", "agents_config")

    config = load_server_config(config_directory=servers_config_directory)

    for server_name in config.keys():
        script_name = f"{server_name.replace('_server', '').replace('_', '-')}-server"
        entry_point = f"agentic_registry_agents.agents.{server_name}.server:main"
