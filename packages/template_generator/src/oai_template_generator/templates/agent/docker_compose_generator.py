#!/usr/bin/env python3.11
import glob
import json
import os
import sys
from typing import Dict, Any

from ruamel.yaml import YAML

yaml = YAML()
yaml.default_flow_style = False
yaml.indent(mapping=2, sequence=4, offset=2)
yaml.width = 1000


def get_current_user_info():
    """
    Get current user's UID and GID.

    Returns:
        tuple: (uid, gid) or (None, None) if not available
    """
    try:
        uid = os.getuid()
        gid = os.getgid()
        return uid, gid
    except (AttributeError, OSError):
        # getuid/getgid not available on Windows or other issues
        return None, None


def ensure_host_directories(uid: int = None, gid: int = None):
    """
    Ensure host directories exist with correct ownership before Docker creates them.

    Args:
        uid (int): User ID to set ownership
        gid (int): Group ID to set ownership
    """
    file_root = os.path.dirname(__file__)
    directories_to_create = [os.path.join(file_root, 'logs')]

    for directory in directories_to_create:
        if not os.path.exists(directory):
            os.makedirs(directory, exist_ok=True)
            print(f"Created directory: {directory}")

            # Set ownership if UID/GID are available and we're on a Unix-like system
            if uid is not None and gid is not None:
                try:
                    os.chown(directory, uid, gid)
                    os.chmod(directory, 0o777)
                    print(f"Set ownership of {directory} to {uid}:{gid}")
                except (OSError, PermissionError) as e:
                    print(f"Warning: Could not set ownership of {directory}: {e}")
                except AttributeError:
                    # os.chown not available on Windows
                    print(f"Note: Cannot set ownership on this platform for {directory}")


def load_server_config(config_directory) -> Dict[str, Any]:
    """
    Load server configuration from JSON and YAML files.

    Returns:
        Dict[str, Any]: Dictionary of server configurations
    """
    server_config = {}
    # Support .json, .yaml, and .yml files
    config_patterns = [
        os.path.join(config_directory, "*.json"),
        os.path.join(config_directory, "*.yaml"),
        os.path.join(config_directory, "*.yml"),
    ]

    config_files = []
    for pattern in config_patterns:
        config_files.extend(glob.glob(pattern))

    for config_file in config_files:
        server_name = os.path.basename(config_file).split('.')[0]
        ext = os.path.splitext(config_file)[1].lower()
        if ext == ".json":
            server_config[server_name] = load_individual_json_config(config_file)
        elif ext in [".yaml", ".yml"]:
            server_config[server_name] = load_individual_yaml_config(config_file)

    return server_config


def load_individual_yaml_config(config_file: str):
    """
    Load individual server configuration from YAML file.

    Args:
        config_file (str): Path to configuration file

    Returns:
        Dict[str, Any]: Configuration data
    """
    try:
        with open(config_file, "r", encoding='utf-8') as file:
            return yaml.load(file)
    except FileNotFoundError:
        print(f"Configuration file not found: {config_file}")
        sys.exit(1)
    except Exception as e:
        print(f"Error loading server configuration: {str(e)}")
        sys.exit(1)


def load_individual_json_config(config_file):
    """Load server configuration from JSON file."""
    try:
        with open(config_file, 'r') as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"Error: Configuration file '{config_file}' not found.")
        sys.exit(1)
    except json.JSONDecodeError as e:
        print(f"Error: Invalid JSON in '{config_file}': {e}")
        sys.exit(1)


def generate_service_config(service_name: str, config: dict) -> dict:
    """Generate Docker Compose service configuration."""
    port = config.get('port')
    auth_enabled = 'true'
    redis_host = 'agent-valkey'
    redis_port = 6379

    # Base environment variables - always include AGENT_NAME
    environment = [f'AGENT_NAME={service_name}']

    # Add environment variables from config
    env_vars = config.get('environment', {})
    for key, value in env_vars.items():
        if ('$' in value) and (('{' in value and '}' in value) or ' ' not in value):
            # Use Docker Compose syntax to pass literal variable name if not defined
            var_name = value.replace('${', '').replace('}', '').replace('$', '')
            environment.append(f'{key}=${{{var_name}:-$${var_name}}}')
        else:
            environment.append(f'{key}={value}')

    aws_region = os.environ.get("AWS_REGION", 'us-east-1')
    if aws_region:
        environment.append(f'AWS_REGION={aws_region}')

    environment.append(f"REDIS_HOST={redis_host}")
    environment.append(f"REDIS_PORT={redis_port}")
    environment.append(f"AGENT_AUTH_ENABLED={auth_enabled}")

    volumes = ['./logs:/tmp']

    aws_exists = os.path.isdir(os.path.expanduser('~/.aws'))
    if aws_exists:
        aws_path = os.path.expanduser('~/.aws')
        volumes.append(f'{aws_path}:/home/agentuser/.aws:ro')

    if 'crewai' in config.get('tags', []):
        docker_file = 'Dockerfile_debian'
    else:
        docker_file = 'Dockerfile'

    service_config = {
        'build': {
            'context': '.',
            'dockerfile': docker_file,
            'args': {
                'GITHUB_TOKEN': '${GITHUB_TOKEN}',
                'GITHUB_URL': f"{config.get('source')}"
            }
        },
        'container_name': f'agent-{service_name.replace("_", "-")}',
        'ports': [f"{port}:{port}"],
        'volumes': volumes,
        'environment': environment,
        'command': f'--port {port}',
        'restart': 'unless-stopped',
        'networks': ['agent-server-network'],
        'healthcheck': {
            'test': ['CMD', 'curl', '-f', f'http://localhost:{port}/health'],
            'interval': '30s',
            'timeout': '10s',
            'retries': 3
        }
    }

    return service_config


def generate_docker_compose(server_config: dict) -> dict:
    """Generate complete Docker Compose configuration."""
    services = {}

    for service_name, config in server_config.items():
        port = config.get('port')
        if port is None:
            print(f"Warning: No port specified for service '{service_name}', skipping...")
            continue

        services[service_name] = generate_service_config(service_name, config)

    services['valkey'] = {
        'image': 'valkey/valkey:latest',
        'container_name': 'agent-valkey',
        'environment': {
            'REDIS_PASSWORD': 'admin',
        },
        'networks': ['agent-server-network'],
        'command': 'valkey-server --appendonly yes',
        'ports': [
            '6379:6379'
        ],
        'volumes': [
            'valkey-agent-data:/data'
        ],
    }

    # Environment variables are already handled per service in generate_service_config

    return {
        'services': services,
        'networks': {
            'agent-server-network': {
                'driver': 'bridge',
                'ipam': {
                    'config': [
                        {'subnet': '172.25.0.0/16'}
                    ]
                }
            }
        },
        'volumes': {
            'valkey-agent-data': {}
        }
    }


def write_docker_compose(compose_config: dict, output_file: str = 'docker-compose.yaml'):
    """Write Docker Compose configuration to YAML file."""
    try:
        with open(output_file, 'w') as f:
            yaml.dump(compose_config, f)
        print(f"Docker Compose file generated successfully: {output_file}")
    except Exception as e:
        print(f"Error writing Docker Compose file: {e}")
        sys.exit(1)

def create_log_directory():
    uid, gid = get_current_user_info()
    if uid is not None and gid is not None:
        print(f"Auto-detected UID: {uid}, GID: {gid} for all services")
    else:
        print("Warning: Could not determine UID/GID, services will run as default user")

    if uid is not None and gid is not None:
        # Pre-create host directories with correct ownership
        ensure_host_directories(uid, gid)


def main():
    """Main function to generate Docker Compose file."""
    # Default configuration file
    config_directory = 'agentic_registry_agents/agents_config'
    output_file = 'docker-compose.yaml'

    # Parse command line arguments
    if len(sys.argv) > 1:
        config_file = sys.argv[1]
    if len(sys.argv) > 2:
        output_file = sys.argv[2]

    print(f"Loading configuration from: {config_directory}")

    # Load server configuration
    server_config = load_server_config(config_directory=config_directory)

    create_log_directory()

    # Generate Docker Compose configuration
    compose_config = generate_docker_compose(server_config)

    # Write to file
    write_docker_compose(compose_config, output_file)

    # Print summary
    print(f"\nGenerated services:")
    for service_name, config in server_config.items():
        port = config.get('port', 'N/A')
        print(f"  - {service_name}: port {port}")


if __name__ == '__main__':
    main()
